...
template <int N_POS_DIMS, int N_FEATURES_PER_LEVEL, int WIDTH, int N_ITERS, typename Encoder>
__device__ void threadblock_load_input_static_with_encoding(__half* __restrict__ act_shmem, const float* __restrict__ coordinate, Encoder& encoder) {
  using namespace TCNN_NAMESPACE;
  ...
  #pragma unroll
  for (int i = 0; i < N_ITERS; ++i) {
    const uint32_t k = row + 16 * i;

    __half* encoded = &act_shmem[lane_offset + k * (WIDTH + SKEW)];

    // We are also initializing encoded output to be zero (important).
    *(int4*)encoded = {0};
    ...
    for (uint32_t l = 0; l < N; ++l) {
      ...
      if (level < encoder.num_levels) encode(encoder, level, coord, output_per_level);
    }
  }
}
...
template <int N_POS_DIMS, int N_FEATURES_PER_LEVEL, int WIDTH, int N_ITERS, typename OUT_T, typename INPUT_LAYOUT, typename Encoder>
__device__ void threadblock_input_layer_forward_dynamic_with_encoding(Activation activation, __half* __restrict__ act_shmem, const __half* __restrict__ weights_this_layer, const uint32_t in_width, const uint32_t batch_size, const float* __restrict__ coordinate, Encoder& encoder) {
  using namespace TCNN_NAMESPACE;
...
template <int N_POS_DIMS, int WIDTH, int N_ITERS>
__device__ void threadblock_read_pos_from_shmem(float* __restrict__ pos_shmem, const float3& pos) {
  ...
  const uint32_t i = threadIdx.x + threadIdx.y * blockDim.x;
